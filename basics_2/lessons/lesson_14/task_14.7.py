...
# Эта задача повышенной сложности, поэтому у вас может не получиться решить её с первого раза.
# Не переживайте, мы подготовили подсказку, которая поможет идти в правильном направлении.
# Постарайтесь выполнить задачу самостоятельно, а если будет нужно — воспользуйтесь подсказкой.

# Для решения этой задачи необходимо создать декоратор counter, который добавит атрибут счётчика
# к функции-обёртке. Функция-обёртка должна быть предназначена для подсчёта количества вызовов
# оригинальной функции. Атрибут счётчика можно добавить с помощью точечной нотации
# имя_функции.новый_атрибут = значение.
# Для реализации декоратора counter:
# Создайте функцию-декоратор counter, принимающую в качестве аргумента функцию, которую нужно декорировать.
# Внутри декоратора создайте функцию-обёртку wrapper, которая будет вызываться вместо оригинальной функции.
# В функции-обёртке wrapper используйте атрибут самой функции-обёртки для хранения и увеличения счётчика вызовов.
# После каждого вызова оригинальной функции увеличивайте счётчик и выводите его значение.
# Не забудьте инициализировать счётчик вызовов перед возвратом функции-обёртки.
from functools import wraps
from typing import Callable, Any
